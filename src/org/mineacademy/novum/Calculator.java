...
public class Calculator {
	...
	private void runCalculator() {
		...
		Calculator.Mode mode = Calculator.Mode.LEFT_HAND_NUMBER;
		double[] numbers = new double[2];

		while (!message.equals("exit")) {
			if (mode == Calculator.Mode.LEFT_HAND_NUMBER)
				System.out.println("Please type the first number.");

			else if (mode == Calculator.Mode.RIGHT_HAND_NUMBER)
			...
			if (mode == Calculator.Mode.LEFT_HAND_NUMBER) {
				if (isNumberValid(message)) {
					...
					mode = Calculator.Mode.RIGHT_HAND_NUMBER;
				} else
				...
				if (isOperatorValid(message)) {
					...
					mode = Calculator.Mode.LEFT_HAND_NUMBER;
				} else
